...
class Empleado extends Model
{
    ...
    {
        return $this->row("SELECT * FROM app_empleados WHERE emp_username='{$user}'");
    }

    public function getUserPerData($id)
    {
        return $this->row("SELECT * FROM app_empleados_info WHERE einfo_id={$id}");
    }

    public function totalEmpleados()
    {
        return $this->count('SELECT * FROM app_empleados WHERE emp_eliminado != 1');
    }

    public function empleadosDatosCompletos()
    {
        return $this->all('SELECT * FROM app_empleados_info ei JOIN app_empleados e ON ei.einfo_id = e.emp_edatosid JOIN app_empleados_sucursales es ON es.empleado_id = e.emp_edatosid JOIN app_sucursales s ON es.sucursal_id = s.su_id');
    }

    public function empleadosInfo()
    {
        return $this->all('SELECT * 
                            FROM app_empleados_info ei 
                            LEFT JOIN app_empleados e ON ei.einfo_id = e.emp_edatosid
                            WHERE einfo_eliminado!=1');
    }

    public function guardarEmpleadoInfo($data)
    {
        return $this->ejecutar("INSERT INTO app_empleados_info (einfo_nombres, einfo_apellidos, einfo_telefono_movil, einfo_email, einfo_direccion_1, einfo_direccion_2, einfo_ciudad, einfo_estado, einfo_pais) VALUES ('{$data['einfo_nombres']}', '{$data['einfo_apellidos']}', '{$data['einfo_telefono_movil']}', '{$data['einfo_email']}', '{$data['einfo_direccion_1']}', '{$data['einfo_direccion_2']}', '{$data['einfo_ciudad']}', '{$data['einfo_estado']}', '{$data['einfo_pais']}')");
    }

    public function editarEmpleadoInfo($id_empleado, $data)
    {
        return $this->ejecutar("UPDATE app_empleados_info SET einfo_nombres='{$data['einfo_nombres']}', einfo_apellidos='{$data['einfo_apellidos']}', einfo_telefono_movil='{$data['einfo_telefono_movil']}', einfo_email='{$data['einfo_email']}', einfo_direccion_1='{$data['einfo_direccion_1']}', einfo_direccion_2='{$data['einfo_direccion_2']}', einfo_ciudad='{$data['einfo_ciudad']}', einfo_estado='{$data['einfo_estado']}', einfo_pais='{$data['einfo_pais']}' WHERE einfo_id = $id_empleado");
    }
    ...
    {
        return $this->row("SELECT emp_estatus FROM app_empleados WHERE emp_edatosid = $id_empleado");
    }

    public function modificarEstatus($id_empleado, $estatus)
    {
        return $this->ejecutar("UPDATE app_empleados SET emp_estatus = $estatus WHERE emp_edatosid = $id_empleado");
    }

    public function eliminarEmpleado($id_empleado)
    {
        return $this->ejecutar("UPDATE app_empleados_info 
                                SET einfo_eliminado = 1 
                                WHERE einfo_id = $id_empleado");
    }
    ...
    public function store_update($person_data, $emp_id, $employee_data, $permission_data, $permission_action_data): void
    {
        ...
        if (is_bool($success) && true === $success) {
            ...
            if (null !== $permission_data) {
                ...
                $this->ejecutar("DELETE FROM app_permissions WHERE emp_id=$emp_id");

                // Ahora insertamos los nuevos permisos
                foreach ($permission_data as $modulos_permitidos) {
                    $this->ejecutar("INSERT INTO app_permissions (module_id,emp_id) VALUES ('{$modulos_permitidos}','{$emp_id}')");
                }

                //Eliminamos tambien las acciones que tiene el usuario
                $this->ejecutar("DELETE FROM app_permissions_actions WHERE emp_id=$emp_id");

                //Ahora insertamos los nuevos permisos
                foreach ($permission_action_data as $permission_action) {
                    list($module, $action) = explode('|', $permission_action);
                    echo $sql = "INSERT INTO app_permissions_actions (module_id,action_id,emp_id) VALUES ('{$module}','{$action}','{$emp_id}')";
                    $this->ejecutar("INSERT INTO app_permissions_actions (module_id,action_id,emp_id) VALUES ('{$module}','{$action}','{$emp_id}')");
                }
            }
        }
    }
    ...
    {
        $success = $this->ejecutar("UPDATE app_empleados_info
										SET
										...
										einfo_imagenid 			= 1
										WHERE einfo_id			= $emp_id");

        return $success;
    }

    public function exists($emp_id): int
    {
        $sql = "SELECT * FROM app_empleados WHERE emp_id = '{$emp_id}'";

        return Model::count($sql);
    }
